...
function generateChangelog(version) {
  return `## ⚡ Premium Download Manager v${version} — Resilient Download Architecture & Modal Fix

### 🌟 الإصلاحات والترقيات في هذا الإصدار / Highlights & Bug Fixes:
- **إصلاح خطأ نافذة إضافة التحميل (Fix: Modal Download \`tasks:add\` Error)**:
  - حل مشكلة \`Error invoking remote method 'pdm': Error: أمر غير معروف: tasks:add\` التي كانت تظهر عند بدء التحميل أو الضغط على "تحميل لاحقاً" من نافذة التأكيد.
  - دعم جميع القنوات النمطية المباشرة في طبقة التوافق العكسي \`pdm\` (\`tasks:add\`, \`tasks:list\`, \`tasks:pause\`, \`tasks:resume\`, إلخ).
  - تعزيز مسار الاستدعاء في \`preload.js\` ليعمل بالتوجيه المباشر والاحتياطي المزدوج مع حماية تامة من أي أخطاء قنوات غير مسجلة.
- **الحفاظ الكامل على ترويسات وبيانات التنزيل القادمة من المتصفح**:
  - الحفاظ على كوكيز الجلسة (\`cookies\`) ومصدر الإحالة (\`referer\`) وهوية المتصفح (\`user-agent\`) عند فتح نافذة التأكيد وتسليمها لمحرك التنزيل.
  - دعم متكامل لزر "تحميل لاحقاً" (\`paused: true\`) بحيث تضاف المهمة متوقفة مؤقتاً في القائمة دون بدء التحميل الفوري.
...
- **واجهة إضافة نظيفة بدون شارة ON ومؤشر دقيق بالداخل (Clean Toolbar & Rebuilt Popup)**:
  - إزالة شارة \`ON\` الخارجية من على أيقونة الإضافة في شريط أدوات المتصفح لتظل نظيفة ومريحة للعين.
...
- **فحص شامل بنسبة 100% لكافة مسارات التنزيل (31 جناح فحص و 211 اختبار مؤكد)**:
  - نجاح كافة اختبارات محرك التنزيل، التورنت، الفيديو، استخراج الصوت، كشف التكرار، والتحميل المباشر والتأكيدي.
