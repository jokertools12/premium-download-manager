...
function generateChangelog(version) {
  return `## ⚡ Premium Download Manager v${version} — Instant Window Pop & Direct Download Toggle

### 🌟 الإصلاحات والترقيات في هذا الإصدار / Highlights & Bug Fixes:
- **فتح فوري لنافذة التحميل من أول نقرة (Instant First-Click Modal Pop < 50ms)**:
  - حل مشكلة تأخر فتح نافذة التأكيد أو الحاجة للنقر المتكرر على الرابط بسبب دخول Service Worker في المتصفح في وضع الخمول (Cold Sleep).
  - تسليم الروابط فورياً إلى المحرك عبر اتصال مباشر دون فحص ping مسبق مع ضبط مهلة الكوكيز بسباق زمني 150ms كحد أقصى.
- **تجاوز قفل التركيز في نظام ويندوز (Windows Foreground Lockout Bypass)**:
  - إصلاح بقاء نافذة البرنامج في الخلفية خلف المتصفح مع وميض أيقونة شريط المهام باللون البرتقالي عند النقر على رابط في كروم.
  - إجبار النافذة على الظهور فوراً في المقدمة أعلى نافذة المتصفح دون أي تأخير أو تداخل.
- **التعبئة الفورية للبيانات في نافذة التأكيد (0ms Instant Data Population)**:
  - استخراج اسم الملف فورياً وتصنيف المجلد تلقائياً (فيديو، برامج، صوتيات، مستندات، أرشيف) وعرض بطاقة الفحص في 0ms دون تجميد النافذة أو انتظار فحص الشبكة.
  - تخفيض مهلة فحص الروابط بالشبكة LinkInspector إلى 2.5 ثانية فقط كحد أقصى مع استمرار إمكانية البدء فوراً.
- **خيار التحميل المباشر وتجاوز النافذة (Direct Download Bypass Option)**:
  - إضافة خيار مباشر داخل نافذة التأكيد: \`التحميل المباشر دائماً وتجاوز هذه النافذة مستقبلاً\` للتبديل الفوري بين نمط النافذة والتحميل المباشر.
  - إمكانية التحكم الكامل في النمط في أي وقت من شاشة الإعدادات (\`بدء التنزيل فوراً عند الاستلام من المتصفح دون إظهار نافذة التأكيد\`).
- **فحص شامل واختبارات تأكيدية بنسبة 100% (32 جناح فحص و 217 اختباراً ناجحاً)**:
  - تغطية شاملة لكافة أنماط التحميل المباشر والتأكيدي، اعتراض المتصفح، منع التنزيل المزدوج، وقفل تركيز النوافذ.
